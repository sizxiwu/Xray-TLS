...
# --- 新增：国内IP检测与镜像设置 ---
MIRROR_PREFIX=""
COUNTRY=$(curl -s --connect-timeout 2 ipinfo.io/country)
if [ -z "$COUNTRY" ]; then
    # 如果ipinfo.io超时或失败，尝试ip.sb作为备用
    COUNTRY=$(curl -s --connect-timeout 2 ip.sb/country)
fi

if [ "$COUNTRY" = "CN" ]; then
    echo -e "${yellow}检测到您是中国大陆IP，将使用镜像加速下载...${plain}"
...
install_V2bX() {
    ...
    if  [ $# == 0 ] ;then
        ...
        # --- 修改：应用镜像前缀 ---
        wget --no-check-certificate -N --progress=bar -O /usr/local/V2bX/V2bX-linux.zip "${MIRROR_PREFIX}https://github.com/wyx2685/V2bX/releases/download/${last_version}/V2bX-linux-${arch}.zip"
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        # --- 修改：应用镜像前缀 ---
        url="${MIRROR_PREFIX}https://github.com/wyx2685/V2bX/releases/download/${last_version}/V2bX-linux-${arch}.zip"
        echo -e "开始安装 V2bX $1"
...
EOF
    ...
    # --- 修改：应用镜像前缀 ---
    curl -o /usr/bin/V2bX -Ls "${MIRROR_PREFIX}https://raw.githubusercontent.com/wyx2685/V2bX-script/master/V2bX.sh"
    ...
    if [[ $first_install == true ]]; then
        ...
        if [[ $if_generate == [Yy] ]]; then
            # --- 修改：应用镜像前缀 ---
            curl -o ./initconfig.sh -Ls "${MIRROR_PREFIX}https://raw.githubusercontent.com/wyx2685/V2bX-script/master/initconfig.sh"
